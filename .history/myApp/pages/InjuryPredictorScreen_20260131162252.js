...
export default function InjuryPredictor({ navigation }) {
 ...
 // Replace the handleAnalyzeInjury function with this:
...
const handleAnalyzeInjury = async () => {
  ...
  try {
    ...
    if (image) {
      try {
        // Method 1: Using FileSystem.readAsStringAsync (newer API)
        const base64 = await FileSystem.readAsStringAsync(image, {
          encoding: 'base64',
        });
        base64Image = base64;
      } catch (fsError) {
        console.log("FileSystem error, trying alternative:", fsError);
        
        // Method 2: Try using fetch to get base64
        try {
          ...
          console.log("Fetch conversion failed:", fetchError);
          // Continue without image - AI will still work with symptoms and description
          Alert.alert(
            "Image Processing Warning",
            "Could not process the image, but AI will analyze based on symptoms and description.",
            [{ text: "OK" }]
          );
        }
      }
