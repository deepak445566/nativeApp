...
export default function ActiveWorkoutScreen({ route, navigation }) {
  ...
  useEffect(() => {
    ...
    // ✅ Setup app state listener for automatic pause
    ...
    return () => {
      ...
      backHandler.remove();
      subscription.remove(); // ✅ Cleanup app state listener
    };
  }, []);

  // ✅ Handle app state changes (screen switch, minimize, etc.)
  ...
  // ✅ Pause workout when component loses focus
  useEffect(() => {
  ...
  // ✅ Resume workout when component gains focus
  useEffect(() => {
    const unsubscribe = navigation.addListener('focus', () => {
      if (isPaused) {
        // Optionally resume if user wants
        // Or keep paused and let user manually resume
      }
    });
  ...
  const pauseWorkout = () => {
    if (!isPaused) {
      ...
      console.log('Workout auto-paused due to screen switch');
    }
  };
      ...
      >
        ...
        <View style={styles.videoSection}>
          <View style={styles.videoContainer}>
            <VideoPlayer
              onClose={() => {
                pauseWorkout(); // ✅ Pause before closing
                navigation.goBack();
              }}
