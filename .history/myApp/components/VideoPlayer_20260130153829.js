// components/VideoPlayer.js (Updated with expo-video)
...
export default function VideoPlayer({ 
    ...
    onPanResponderMove: (_, gestureState) => {
      if (status.durationMillis) {
        const newPosition = Math.max(0, Math.min(
          status.positionMillis + (gestureState.dx / width) * status.durationMillis,
  ...
  if (error) {
    return (
      <SafeAreaView style={[styles.container, { height: VIDEO_HEIGHT }]}>
  ...
  return (
    <SafeAreaView style={[
      ...
      <Video
        ref={videoRef}
        source={videoSource || getVideoSource()}
